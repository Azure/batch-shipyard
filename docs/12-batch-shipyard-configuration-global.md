...
Docker Hub login information in the credentials config. To specify a private
registry other than on Docker Hub, a property named `server` should be
defined. To use a private registry backed by Azure Blob Storage, define a
property named `azure_storage`. Note that a maximum of only one of these
...
describes members of the non-Docker Hub private registries supported:
  * (optional) `server` object is a property that is the fully-qualified host
    ...
    in the credentials config file.
